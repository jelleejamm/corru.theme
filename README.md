...
- `corru.theme.interloper.css` makes your messages appear like the interloper's. **themeattributes required**
- `corru.theme.selfcolor.css` lets you customize your messages' colors independently by modifying the `--corrutheme-chat-selfcolor` attribute in the `:root` element. incompatible with the interloper theme, **themeattributes required**
- `corru.theme.selfmsgright.css` moves your messages to the right, similarly to the interloper's. **themeattributes required**
- `corru.theme.akizet.css` is an example custom theme which uses `corru.theme.selfcolor.css` to turn your messages into akizet's. **themeattributes required**
- `corru.theme.obesk.css` is an example custom theme which turns messages in chat and search results to match that of obeski
